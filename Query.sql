...
After LIKE, you could indicate what you are looking for with or without using parentheses. Both options are correct 
and will deliver the same output. We think using parentheses is better for legibility and that’s why it is the first option we’ve suggested.*/
...
FROM 
	salaries
WHERE
	salary >= 100000 AND datediff(to_date, from_date) > 365;
...
DELIMITER $$
CREATE TRIGGER checkHireDate BEFORE INSERT ON employees 
FOR EACH ROW
BEGIN
	DECLARE today date;
...
DROP FUNCTION IF EXISTS largestContractSalary;

DELIMITER $$
CREATE FUNCTION largestContractSalary(p_emp_no INTEGER) RETURNS DECIMAL(10,2)
DETERMINISTIC
BEGIN

DECLARE largestContractSalary DECIMAL(10,2);

SELECT
	MAX(s.salary)
INTO largestContractSalary FROM
    employees e
    JOIN salaries s ON e.emp_no = s.emp_no
WHERE 
	p_emp_no = e.emp_no;
RETURN largestContractSalary;
END$$

DELIMITER ;

SELECT largestContractSalary(11356);

/*Exercise 10
Based on the previous exercise, you can now try to create a third function that also accepts a second parameter. 
Let this parameter be a character sequence. Evaluate if its value is 'min' or 'max' and based on that retrieve either 
the lowest or the highest salary, respectively (using the same logic and code structure from Exercise 9). If the inserted 
value is any string value different from ‘min’ or ‘max’, let the function return the difference between the highest and the 
lowest salary of that employee.*/

DROP FUNCTION IF EXISTS f_salary;

DELIMITER $$
CREATE FUNCTION f_salary(p_emp_no INTEGER, p_min_or_max VARCHAR(10)) 
